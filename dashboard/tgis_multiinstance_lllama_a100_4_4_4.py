...
def get_stats(user):
    ...
    # tgi_request_count = tmp_llama['tgi_request_count'].diff().sum()
    # tgi_request_success = tmp_llama['tgi_request_success'].diff().sum()
    # print(f'TGI Request Count: {tgi_request_count} || TGI REQUEST SUCCESS: {tgi_request_success}')
    ...
    valid_latency = tgis_latency_df.dropna()
    valid_inference = tgis_inference_df.dropna()
    valid_queue_time = tgis_queue_time_df.dropna()

    # Calculate and print percentiles
    print("------------------------------------------")
    print(f'95 %ile latency is: {valid_latency.quantile(0.95)}')
    print(f'90 %ile latency is: {valid_latency.quantile(0.90)}')
    ...
    queue_data = {
        'Llama  Queue Time': valid_queue_time,
    ...
    plot_histogram_with_cdf(queue_data, 'Queue Time (ms)', 'Cumulative Probability', f'Queue Time CDF for User {user}', f'images/{user}_queue_time_tgis_4.4.4_llama_a100_time.png')
    plot_histogram_with_cdf(latency_data, 'Latency (ms)', 'Cumulative Probability', f'Latency CDF for User {user}', f'images/{user}_latency_4.4.4_llama_a100_time.png')
...
def plot_histogram_with_cdf(data, xlabel, ylabel, title, filename):
    ...
    for label, df in data.items():
        plt.hist(df, bins=2000, cumulative=True, density=True, histtype='step', label=label)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
