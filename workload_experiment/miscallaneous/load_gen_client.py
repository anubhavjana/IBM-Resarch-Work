...
class OpenShiftClient:
	...
	def create_pod(self, w_type,request_rate):
		...
		try:
			...
			print(f"Create workload pod for rpm: ",request_rate )
		except Exception as e:
			print("Error :", e.body)
	...
	def log_data(self, start_time, end_time, rr):
		"""
		...
		file_name = "llama_vllm_a100.jsonl"
...
def main():
	...
	openshift_client.delete_pod("tgis-grpc-load-vllm-llama")
	...
	for rr in range(10,120,10): # request rate
		...
		openshift_client.create_pod("vllm_llama",rr)
		...
		openshift_client.log_data(start, end, rr)

		openshift_client.delete_pod("tgis-grpc-load-vllm-llama") #experiment over ---  delete pod
