...
def analyze_model(model_name):
    request_rates = {
        'granite': [180, 240, 300, 400],
        'llama': [60, 70, 80, 90, 120]
    }
    ...
    tgi_filters = {
        'granite': {'pod': ['vllm-granite']},
        'llama': {'pod': ['vllm-llama3']},
    }
    ...
    for rate in request_rates[model_name]:
        folder = f'vllm_benchmark_data/vllm_rr_{rate}_{model_name}/tgis_vllm/'
        ...
        tgi_data = tgi_df[['timestamp', 'vllm:gpu_cache_usage_perc', 'vllm:num_requests_running', 'tgi_request_queue_duration_sum', 
                            'tgi_request_queue_duration_count', 'tgi_batch_current_size', 'tgi_queue_size', 
                            'tgi_request_generated_tokens_sum', 'tgi_request_generated_tokens_count',  
                            'tgi_request_duration_sum', 'tgi_request_duration_count', 'tgi_request_input_length_sum', 
                            'tgi_request_input_length_count']]
