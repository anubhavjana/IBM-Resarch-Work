...
tgis_llama_data_folder = f'bloom_a100_no_priority/rr_{rr}_op_{op}/tgis/'
...
users = ['Alan']
if(len(users)==1):
    print(f"Experiment carried out for single user {users[0]} with RPM = {rr} with max output token count {op}")
...
for user in users:
    ...
    axes[0].set_title('CDF of Mean Latency Per Token - Bloom A100')
    ...
    axes[1].set_title('CDF of Queue Time -  Bloom A100')
    ...
    axes[2].set_title('CDF of TGIS E2E Latency -  Bloom A100')
    axes[2].legend(loc='upper left')
    axes[2].grid(True)

    plt.tight_layout()
    plt.savefig(f'Bloom_CDF__rr_{rr}_op_{op}.png')
