...
# data_folder = 'router_experiments_data/no_router_6.6.6/'
...
# tgi_filter_llama_1 = {'pod':['vllm-llama-1']}
# tgi_filter_llama_2 = {'pod':['vllm-llama-2']}
# tgi_filter_llama_3 = {'pod':['vllm-llama-3']}
...
router_queue_df = router_tmp['router_queue_duration_sum'].diff()/router_tmp['router_queue_duration_sum'].diff()
print(f"Router Mean Queue Time for {user}: {router_queue_df.mean()}")
...
def get_stats(user):
    ...
    tgis_latency_df = tmp_llama_1['tgi_request_duration_sum'].diff() / tmp_llama_1['tgi_request_duration_count'].diff()
    tgis_inference_df = tmp_llama_1['tgi_request_inference_duration_sum'].diff() / tmp_llama_1['tgi_request_inference_duration_count'].diff()
    ...
    tgis_latency_df = tmp_llama_2['tgi_request_duration_sum'].diff() / tmp_llama_2['tgi_request_duration_count'].diff()
    tgis_inference_df = tmp_llama_2['tgi_request_inference_duration_sum'].diff() / tmp_llama_2['tgi_request_inference_duration_count'].diff()
    tgis_queue_time_df = tgis_latency_df - tgis_inference_df
    ...
    tgis_latency_df = tmp_llama_3['tgi_request_duration_sum'].diff() / tmp_llama_3['tgi_request_duration_count'].diff()
    tgis_inference_df = tmp_llama_3['tgi_request_inference_duration_sum'].diff() / tmp_llama_3['tgi_request_inference_duration_count'].diff()
    tgis_queue_time_df = tgis_latency_df - tgis_inference_df
    ...
    print(f"Router Success: {router_request_success_count} -- Router Failure {router_request_failure_count}")
